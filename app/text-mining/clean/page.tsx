...
import ViewCleanResultPopup from "@/components/popup/ViewCleanResultPopup";
...
const CleanPage = () => {
  const [itemList, setItemList] = useState<CleanListItem[]>([]);
  ...
  const [filterExtractType, setFilterExtractType] = useState("all");
  ...
  const [viewItem, setViewItem] = useState<CleanListItem | null>(null);
  ...
  useEffect(() => {
    ...
    if (itemList.length > 0) {
      console.log("First item extract flags: ", {
        extractNoun: itemList[0].extractNoun,
        extractVerb: itemList[0].extractVerb,
        extractAdjective: itemList[0].extractAdjective,
      });
    }
  }, [itemList]);
  ...
  const filteredData = useMemo(() => {
    return itemList
      .filter(item => {
        ...
        // Extract Type 필터링
        if (filterExtractType !== "all") {
          switch (filterExtractType) {
            case "noun":
              if (!item.extractNoun) return false;
              break;
            case "adjective":
              if (!item.extractAdjective) return false;
              break;
            case "verb":
              if (!item.extractVerb) return false;
              break;
            case "none":
              if (item.extractNoun || item.extractAdjective || item.extractVerb) return false;
              break;
          }
        }
      ...
      .sort((a, b) => {
        // 정렬
        let aValue = a[sortField as keyof CleanListItem];
        let bValue = b[sortField as keyof CleanListItem];
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200">
        ...
        <div className="p-6 border-b border-gray-200">
          <div className="grid grid-cols-1 md:grid-cols-7 gap-4">
            ...
            <div>
              <label htmlFor="extract-type-filter" className="block text-sm font-medium text-gray-700 mb-2">
                Extract Type
              </label>
              <select
                id="extract-type-filter"
                className="block w-full border-gray-300 rounded-lg shadow-sm text-sm px-3 py-2 border focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                value={filterExtractType}
                onChange={e => setFilterExtractType(e.target.value)}
              >
                      ...
                      <tr key={item.id} className="hover:bg-gray-50 transition-colors cursor-pointer">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          <div className="space-y-1">
                            ...
                            {!item.extractNoun && !item.extractAdjective && !item.extractVerb && (
