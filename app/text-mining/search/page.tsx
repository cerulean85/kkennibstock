...
import ViewSearchResultPopup from "@/components/popup/ViewSearchResultPopup";
...
const SearchPage = () => {
  ...
  const [viewItem, setViewItem] = useState<SearchListItem | null>(null);
  ...
  return (
    <div className="space-y-8 px-2 pt-3 min-h-screen">
      ...
      <DashboardHeader
        ...
        description={`Manage and monitor your text mining tasks`}
        onRefresh={() => {
          fetchData();
          setRefreshCountdown(30);
        }}
                  ...
                  <tbody className="bg-blue-500 divide-y divide-gray-200">
                    {paginatedData.map(item => (
                      <tr key={item.id} className="hover:bg-gray-50 transition-colors cursor-pointer">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.searchStartDate}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.searchEndDate}</td>
