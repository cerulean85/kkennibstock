...
export class MemberRepository {
  ...
  async logout(userSeqId: number, refreshToken: string) {
  ...
  async getUserList(startDate: string, endDate: string, keyword: string, page: number, limit: number) {
    const res: any = await fetchGet(
      `users?startDate=${startDate}&endDate=${endDate}&keyword=${keyword}&page=${page}&limit=${limit}`,
      "getUserList"
    );
    return res;
  }
  ...
  async getLoginHistoryList(startDate: string, endDate: string, keyword: string, page: number, limit: number) {
    const res: any = await fetchGet(
      `history?startDate=${startDate}&endDate=${endDate}&keyword=${keyword}&page=${page}&limit=${limit}`,
      "getLoginHistoryList"
    );
    return res;
  }
  ...
  async sendContactUs(name: string, email: string, message: string): Promise<any> {
